...
1 安装 librrd: sudo apt install librrd-dev
2 安装 pepper_bus: go get github.com/johntech-o/pepperbus/
2 安装运行 go_keeper: env GIT_TERMINAL_PROMPT=1 go get git.huajiao.com/qmessenger/gokeeper

- 用 pepper_bus 下的 keeper.conf 替换/usr/local 下 gokeeper 的 keeper.conf
  cp /{your_qbus_project}t/src/github.com/johntech-o/pepperbus/etc/keeper.conf /usr/local/gokeeper/keeper.conf

* 新建 domain 并复制 dev 下的 conf 文件到新的 domain 下
  mkdir /usr/local/gokeeper/data/config/mydomain
  cp -r /{your_qbus_project}/src/github.com/johntech-o/pepperbus/etc/dev/\* /usr/local/gokeeper/data/config/mydomain/
...
- 用 pepper_bus 你的 php-fpm.conf 替换原有的配置 cp /{your_qbus_project}/src/github.com/johntech-o/pepperbus/etc/php-fpm.conf /etc/php/7.0/fpm/php-fpm.conf
- 重启 php-fpm sudo service php7.0-fpm restart

4 安装运行 redis

- 安装 redis sudo apt install redis-server
- cp /{your_qbus_project}/src/github.com/johntech-o/pepperbus/etc/redis.conf /etc/redis/redis.conf
...
- 配置文件为 /{your_qbus_project}/src/github.com/johntech-o/pepperbus/etc/dev/queue.conf
  设置需要测试的队列的消费文件：
  topic_cgi_script_entry map[string]string = queue1-topic1:{your_project_path}/src/github.com/johntech-o/pepperbus/example/php/consume.php,queue1-topic2:{your_project_path}/src/github.com/johntech-o/pepperbus/example/php/consume.php,queue2-topic3:/home/q/system/hello/index.php 修改为你的消费 php 文件
- 用新文件替换对应的 mydomain 下的配置文件
- 重启 gokeeper

2 运行一次

- **添加文件的 demo:** /Users/johntech/code/gocode/src/github.com/johntech-o/pepperbus/example/php/product.php
