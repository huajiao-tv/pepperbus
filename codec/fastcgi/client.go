...
func (this *FCGIClient) SetReadDeadline(d time.Duration) error {
	return this.conn.SetReadDeadline(time.Now().Add(d))
}

func (this *FCGIClient) SetWriteDeadline(d time.Duration) error {
	return this.conn.SetWriteDeadline(time.Now().Add(d))
}
...
func (this *FCGIClient) writeBeginRequeost(role uint16, flags uint8) error {
	b := [8]byte{byte(role >> 8), byte(role), flags}
	return this.writeRecord(FCGI_BEGIN_REQUEST, b[:])
}
...
func (this *FCGIClient) Do(p map[string]string, req io.Reader) (r io.Reader, err error) {
	...
	err = this.writeBeginRequest(uint16(FCGI_RESPONDER), flag)
	if err != nil {
		return
	}
